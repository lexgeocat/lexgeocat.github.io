...
comment on
table normativa_grupos is 'Los 8 grupos romanos del índice normativo (I..VI, VIII, IX — sin VII por diseño).';
...
create policy "normativa_grupos_select_public" on normativa_grupos for
select using (true);

drop policy if exists "normativa_grupos_write_auth" on normativa_grupos;

create policy "normativa_grupos_write_auth" on normativa_grupos for all to authenticated using (
...
comment on
table normativa_tipos is 'Los 20 tipos de norma del índice, cada uno asociado a su grupo romano.';
...
create policy "normativa_tipos_select_public" on normativa_tipos for
select using (true);

drop policy if exists "normativa_tipos_write_auth" on normativa_tipos;

create policy "normativa_tipos_write_auth" on normativa_tipos for all to authenticated using (
    auth.role () = 'authenticated'
)
with
    check (
...
create policy "normativa-pdfs_update_auth" on storage.objects for
update to authenticated using (bucket_id = 'normativa-pdfs')
with
    check (bucket_id = 'normativa-pdfs');
