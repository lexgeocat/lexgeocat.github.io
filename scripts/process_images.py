...
Tras procesar, borra el archivo original de raw-uploads/.
"""
...
# Forzar UTF-8 en stdout/stderr (Windows cp1252 rompe con caracteres no ASCII)
try:
...
def slug_from_filename(name: str) -> str | None:
    """Quita el prefijo de timestamp y la extensión.

    Worker: <timestamp>-<safeName>.<ext> → <safeName>.webp
    p.ej.: 1782011606159-p.jpeg → p.webp
    """
...
def process_destino(destino: str) -> tuple[int, list[Path]]:
    ...
    for entry in sorted(src_dir.iterdir()):
        ...
        if not slug:
            print(f"  [skip] {entry.name} (no tiene prefijo de timestamp esperado)")
            continue
        ...
        os.remove(entry)
        print(f"  [ok] {entry.name} -> {dest.relative_to(REPO_ROOT)}")
        processed += 1
...
def main() -> int:
    ...
    if all_failed:
        print(f"[warn] {len(all_failed)} archivo(s) con error (no se borraron)", file=sys.stderr)
        return 1
    return 0
