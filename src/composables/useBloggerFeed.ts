...
function detectCat(labels: any[]): { cls: string; label: string } {
...
function parseEntry(e: any, overrideCls?: string, overrideLabel?: string): BlogEntry {
  ...
  const url = (e.link || []).find((l: any) => l.rel === 'alternate')?.href || SITE.blog.url
  ...
  const excerpt = html.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 130)
  const date = e.published?.$t
    ? new Date(e.published.$t).toLocaleDateString('es', { year: 'numeric', month: 'short', day: 'numeric' })
    : ''
  const cat = overrideCls && overrideLabel
  ...
  async function load() {
    ...
    try {
      ...
      entries.value = raw
        ...
        .map((e: any) => parseEntry(e, options.categoryCls, options.categoryLabel))
    } catch (e: any) {
      error.value = e.message || 'Error cargando artículos'
    } finally {
      loading.value = false
    }
  }
